...
def load_data(config):
    ...
    if data_name in ['Scene_15']:
        ...
        train_x = []
        # 稀疏矩阵sparse csr_matrix从行开始数，行优先 .A将矩阵转换为数组
        ...
        # random.sample 随机截取列表
        index = random.sample(range(train_x[0].shape[0]), 2100)
        for view in [1, 2]:
            # numpy花式索引 取index中的行数 返回的是ndarray
        ...
        for view in [3, 4]:
            ...
            X_list.append(x)
        # squeeze 从数组的形状中删除单维度条目，即把shape中为1的维度删除
        ...
        X_list.append(data1)
        X_list.append(data2)
        y = np.squeeze(mat["label"].T)
        Y_list.append(y)
    elif data_name in ['Reuters_dim10']:
        mat = sio.loadmat(os.path.join(main_dir, 'data', data_name + '.mat'))
        X_list.append(normalize1(np.vstack((mat['x_train'][0], mat['x_test'][0]))))
        X_list.append(normalize1(np.vstack((mat['x_train'][1], mat['x_test'][1]))))
